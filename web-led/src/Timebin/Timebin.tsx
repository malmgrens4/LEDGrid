...
import { hexToRGB, RGBToHex } from "../PixelGrid/GridHelpers";
...
const Snapshot = ({
  ...
  return (
    <div className={classes.snapshot}>
      <div
        className={classes.closeSnapshot}
        onClick={(event) => {
        ...
        >
          <Grid item xs={12}>
            <div className={`${classes.snapshotDisplay}`}>
              {snapshots.length > 0 && snapshots[simulationIndex] && (
                <div className={classes.snapshotDisplay}>
                  <JustGrid
                    cells={snapshots[simulationIndex].cells}
                    cols={cols}
                    rows={rows}
                  />
                </div>
...
const scrollToRef = (
  innerElRef: RefObject<any>,
  parentElRef: RefObject<any>
) => {
...
export const Timebin = ({
  ...
  useEffect(() => {
    let i = 0;
    let loopTimeout: undefined | NodeJS.Timeout;
    if (loop && snapshots.length > 0) {
  ...
  const execute = async () => {
    ...
    for (let i = snapshots.length - 1; i > -1; i--) {
      ...
      if (fadeMode && i > 0) {
        ...
        let fadeTime = duration / fadeCount;
        for (let j = 0; j < fadeCount; j++) {
          let fadeCells: string[] = [];
          for (let k = 0; k < cells.length; k++) {
            let startColor = cells[k];
            let nextSnapshot = i == 0 ? snapshots.length - 1 : i - 1;
            let endColor = snapshots[nextSnapshot].cells[k];
            let alpha = (j + 1) / fadeCount;
            const { red, green, blue } = hexToRGB(startColor);
            const { red: endRed, green: endGreen, blue: endBlue } = hexToRGB(
              endColor
            );
            let newRed = red - Math.floor((red - endRed) * alpha);
            let newBlue = blue - Math.floor((blue - endBlue) * alpha);
            let newGreen = green - Math.floor((green - endGreen) * alpha);
            let frameColor = RGBToHex({
              red: newRed,
              blue: newBlue,
              green: newGreen,
            });
            fadeCells[k] = frameColor;
          }

          setTimeout(() => {
            submitGridRequest(fadeCells);
          }, remainingDuration - fadeTime * j);
        }
        setTimeout(async () => {
          submitGridRequest(cells);
        }, remainingDuration);
        ...
        <Card>
          <CardContent>
            <div className={classes.snapshots}>
              ...
              <div
                ref={addSnapRef}
                className={`${classes.snapshot} ${classes.addSnapshot}`}
                onClick={addSnapshot}
              >
